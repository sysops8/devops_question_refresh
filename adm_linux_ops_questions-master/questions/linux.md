...
Упрощенный порядок загрузки:
    ...
    2. BIOS/UEFI:
        ...
        ◦ Чтение настроек из NVRAM/CMOS (порядок загрузки).
        ◦ Поиск и запуск загрузчика с указанного устройства.
            ▪ BIOS: Ищет код загрузчика в MBR.
            ▪ UEFI: Ищет файл загрузчика (.efi) на разделе EFI.
    3. Загрузчик (GRUB2):
        ◦ Показывает меню (если настроено).
        ◦ Определяет, какое ядро и параметры загрузить (читает /boot/grub/grub.cfg).
        ◦ Загружает выбранное ядро и образ initramfs в оперативную память.
    4. Ядро (Kernel):
        ◦ Распаковывает и использует initramfs как временную корневую ФС.
        ◦ Инициализирует устройства, загружает необходимые драйверы из initramfs.
        ◦ Находит и монтирует настоящий корневой раздел (/).
    5. Initramfs передает управление ядру, которое запускает первый процесс:
        ◦ /sbin/init → в современных системах это systemd (PID 1).
    6. Systemd:
        ◦ Монтирует файловые системы из /etc/fstab.
        ◦ Запускает цели (targets), например basic.target, затем multi-user.target или graphical.target.
        ◦ Параллельно запускает системные службы (юниты), а не "сценарии запуска" в классическом понимании.
    7. Запуск дисплейного менеджера (для графического режима) и появление экрана входа в систему.

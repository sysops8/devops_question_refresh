...
<details>
  ...
  4. Выбор загрузочного устройства - жесткий диск, флешка или PXE (загрузка по сети).
  5. Поиск и загрузка загрузчика (Если UEFI — из EFI System Partition или Если MBR - Grub/LIO/systemd-boot).
  6. Загрузчик определяет ядро и initramfs (Она загружает необходимые драйверы и модули ядра, выполняет начальную инициализацию, а затем передает управление основному процессу init).
  7. Загрузка ядра в память.
  8. Ядро инициализирует свои структуры, запускает initramfs.
  9. Ядро запускает PID 1 (systemd / init).
  10. Запуск сервисов и переход в рабочую систему.

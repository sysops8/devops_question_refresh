...
<details>
  ...
  1. Подача питания / нажатие кнопки включения.   
  2. Инициализация прошивки (BIOS или UEFI) — код прошивки загружается из NVRAM. 
  3. Аппаратная инициализация (POST).
  4. Выбор загрузочного устройства согласно настройкам прошивки.
  5. Если используется UEFI: поиск и запуск EFI-загрузчика на системном разделе EFI (ESP).
  6. Запуск загрузчика (GRUB/systemd-boot/другой).
  7. Выбор и загрузка ядра Linux (и initramfs, если используется).
  8. Инициализация ядра:     
      ◦ настройка драйверов раннего этапа
     
      ◦ монтирование root-файловой системы (часто через initramfs).
  10. Запуск процесса PID 1 (обычно systemd, реже init, OpenRC).
  11. Старт системных служб и юнитов.
  12. Переход в рабочее состояние системы (multi-user, GUI и др.).
</details>
